...
{
    /// <summary>
    /// Azure OpenAI provider using the Chat Completions API on an Azure-hosted deployment.
    ...
    public class AzureOpenAIProvider : IAIProvider
    {
        ...
        /// <param name="apiKey">Your Azure OpenAI resource API key.</param>
        /// <param name="endpoint">Resource endpoint, e.g. "https://my-resource.openai.azure.com".</param>
        /// <param name="deploymentName">Name of the model deployment on the resource.</param>
        /// <param name="apiVersion">Azure OpenAI REST API version.</param>
        public AzureOpenAIProvider(string apiKey, string endpoint, string deploymentName, string apiVersion = "2024-02-15-preview")
        {
            ...
            if (string.IsNullOrWhiteSpace(deploymentName))
                throw new ArgumentException("Azure OpenAI deployment name is required", nameof(deploymentName));
        ...
        {
            if (!IsConfigured)
                return AIResponse.CreateError("Azure OpenAI is not fully configured (endpoint, API key, and deployment name are required)");
            ...
            try
            {
                ...
                if (!httpResponse.IsSuccessStatusCode)
                    return AIResponse.CreateError($"Azure OpenAI API error: {httpResponse.StatusCode} - {TryParseError(responseText)}");
            ...
            CancellationToken cancellationToken = default)
        {
            if (!IsConfigured)
            {
                onError?.Invoke(new InvalidOperationException("Azure OpenAI is not fully configured (endpoint, API key, and deployment name are required)"));
                return;
            }
            ...
            try
            {
                ...
                if (!httpResponse.IsSuccessStatusCode)
                {
                    ...
                    onError?.Invoke(new Exception($"Azure OpenAI API error: {httpResponse.StatusCode} - {TryParseError(errorText)}"));
                    return;
                }
        ...
        {
            return new AzureOpenAIConversation(this);
        }
